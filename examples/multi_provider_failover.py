...
multiple providers (OpenAI → Vertex AI).
...
def example_1_basic_failover():
    """Demonstrate basic two-provider failover."""
    print("\n" + "=" * 70)
    ...
    if os.getenv("OPENAI_API_KEY"):
        providers.append(
            ProviderConfig(
                name="openai", api_key=os.getenv("OPENAI_API_KEY"), model="gpt-4o-mini", priority=1
            )
        )
    ...
    if service_account_file and os.path.exists(service_account_file):
        ...
        print("✓ Vertex AI configured (Priority 2)")
    ...
    response = client.chat_completion(
        messages=[Message(role="user", content="Say hello and tell me which provider you are!")],
        max_tokens=50,
    )
...
def example_2_failover_simulation():
    """Simulate failover when primary provider fails."""
    print("\n" + "=" * 70)
    ...
    providers = [
        ...
        ProviderConfig(
            name="openai", api_key="sk-invalid-key-for-testing", model="gpt-4o-mini", priority=1
        )
    ]
    ...
    if service_account_file and os.path.exists(service_account_file):
        ...
        providers.append(
            ProviderConfig(
                name="openai", api_key=os.getenv("OPENAI_API_KEY"), model="gpt-4o-mini", priority=2
            )
        )
    ...
    config = FlexiAIConfig(
        ...
        circuit_breaker=CircuitBreakerConfig(
            failure_threshold=1, recovery_timeout=60  # Fail fast for demo
        ),
    )
    ...
    try:
        response = client.chat_completion(
            messages=[Message(role="user", content="Hello!")], max_tokens=30
        )
...
def example_3_circuit_breaker_demo():
    ...
    config = FlexiAIConfig(
        providers=providers,
        circuit_breaker=CircuitBreakerConfig(
            failure_threshold=3, recovery_timeout=60, expected_exception="ProviderException"
        ),
    )
    ...
    print("  Failure Threshold: 3 failures")
    print("  Recovery Timeout: 60 seconds")

    # Make several requests
    for i in range(5):
        response = client.chat_completion(
            messages=[Message(role="user", content=f"Request {i+1}: Say hi!")], max_tokens=20
        )
...
def example_4_provider_comparison():
    ...
    for provider_name, model in providers_to_test:
        ...
        if provider_name == "vertexai":
            ...
            provider_config = ProviderConfig(
                name=provider_name, api_key=api_key, model=model, priority=1
            )
        ...
        response = client.chat_completion(
            messages=[Message(role="user", content=prompt)], temperature=0.7, max_tokens=100
        )
...
def example_5_request_statistics():
    """Track request statistics across providers."""
    print("\n" + "=" * 70)
...
def main():
    ...
    if not (has_openai or has_vertexai):
